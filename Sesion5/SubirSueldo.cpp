...
int main () {
  ...
  if (casos_resueltos >= OBJETIVO_CASOS2 && casos_resueltos <= OBJETIVO_CASOS1)     // Se comprueba cuales de los objetivos se
    aumento = aumento + AUMENTO_CASOS2;                                            // han cumplido y se calcula el porcentaje a aumentar.
  ...
  salario_final = salario_final + salario_final * aumento / 100;                    // Se realiza el aumento en el porcentaje calculado anteriormente.
